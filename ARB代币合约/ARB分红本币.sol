...
contract PandaToken is IERC20 {
  ...
  bool private initialized = false;  // 增加初始化标志

// 将constructor替换为initialize函数
function initialize(
    string memory name_,
    ...
    require(!initialized, "Contract is already initialized");  // 确保只能初始化一次
    ...
    // 初始化地址参数
    fundAddress = addressParams[0];  // 用于接收基金的钱包地址
    currency = addressParams[1];     // 代币的交易对币种（如 USDT、WETH 等）
    _swapRouter = ISwapRouter(addressParams[2]); // 路由合约地址
    address ReceiveAddress = addressParams[3]; // 接收代币的初始地址
    ...
    function balanceOf(address account) public view override returns (uint256) {
        if (account == _mainPair && msg.sender == _mainPair && antiSYNC) {
            require(tokenFromReflection(_rOwned[_mainPair]) > 0, "!sync");
        }
